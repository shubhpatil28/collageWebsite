...
export const metadata = {
  ...
  description: "BBA Degree Program at AIM Chalisgaon affiliated with KBCNMU Jalgaon. Business management, marketing, finance, entrepreneurship."
};

export default function BbaProgramPage() {
  const bba = PROGRAMS.find((p) => p.id === "bba") || PROGRAMS[1];

  return (
    <main className="min-h-screen bg-[#FAFAFA]">
      <PageBanner
        title={`${bba.code} - ${bba.name}`}
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-1">
            ...
            <p className="text-sm font-bold text-slate-900">{bba.duration}</p>
          </div>
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-1">
            <span className="text-xs text-slate-500 font-bold uppercase">Affiliation</span>
            <p className="text-sm font-bold text-slate-900">{bba.affiliation}</p>
          </div>
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-1">
            <span className="text-xs text-slate-500 font-bold uppercase">Approval</span>
            <p className="text-sm font-bold text-slate-900">{bba.approval}</p>
          </div>
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold text-slate-900">Career & Higher Education Pathways</h3>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm text-slate-800">
                {bba.careerOpportunities.map((career, idx) => (
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="p-6 rounded-2xl bg-[#0A192F] text-white border border-slate-800 space-y-4">
              <h3 className="text-lg font-bold text-white">Apply For BBA Admission</h3>
